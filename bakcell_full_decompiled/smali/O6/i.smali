.class public final LO6/i;
.super Ld2/r;
.source "SourceFile"


# instance fields
.field public final h:LV7/k2;

.field public final i:I

.field public final j:I

.field public final k:Lse/S;

.field public final l:Lse/M;

.field public final m:Lse/S;

.field public final n:Lse/M;

.field public final o:Lse/Z;

.field public final p:Lse/N;


# direct methods
.method public constructor <init>(LV7/k2;II)V
    .locals 8

    const-string v0, "repo"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/k;->f(Ljava/lang/Object;Ljava/lang/String;)V

    invoke-direct {p0}, Ld2/r;-><init>()V

    iput-object p1, p0, LO6/i;->h:LV7/k2;

    iput p2, p0, LO6/i;->i:I

    iput p3, p0, LO6/i;->j:I

    const/4 p1, 0x0

    const/4 p2, 0x7

    invoke-static {p1, p1, p2}, Lse/T;->b(III)Lse/S;

    move-result-object p3

    iput-object p3, p0, LO6/i;->k:Lse/S;

    new-instance v0, Lse/M;

    invoke-direct {v0, p3}, Lse/M;-><init>(Lse/K;)V

    iput-object v0, p0, LO6/i;->l:Lse/M;

    invoke-static {p1, p1, p2}, Lse/T;->b(III)Lse/S;

    move-result-object p1

    iput-object p1, p0, LO6/i;->m:Lse/S;

    new-instance p2, Lse/M;

    invoke-direct {p2, p1}, Lse/M;-><init>(Lse/K;)V

    iput-object p2, p0, LO6/i;->n:Lse/M;

    const-string p1, ""

    invoke-static {p1}, Lse/T;->c(Ljava/lang/Object;)Lse/Z;

    move-result-object p1

    iput-object p1, p0, LO6/i;->o:Lse/Z;

    new-instance p2, Lse/N;

    invoke-direct {p2, p1}, Lse/N;-><init>(Lse/L;)V

    iput-object p2, p0, LO6/i;->p:Lse/N;

    new-instance v6, LO6/h;

    const/4 p1, 0x0

    invoke-direct {v6, p0, p1}, LO6/h;-><init>(LO6/i;Lkotlin/coroutines/Continuation;)V

    iget-object v1, p0, Ld2/r;->e:Lse/Z;

    const/4 v3, 0x0

    const/16 v7, 0x3e

    const/4 v2, 0x0

    const/4 v4, 0x0

    const/4 v5, 0x0

    move-object v0, p0

    invoke-static/range {v0 .. v7}, Ld2/r;->b(Ld2/r;Lse/L;ZZZZLee/l;I)Lpe/m0;

    return-void
.end method
